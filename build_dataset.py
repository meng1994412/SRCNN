...
for imagePath in imagePaths:
    ...
    # grab the dimensions of the input image and crop the image such that
    # it tile nicely when we generate the training data + labels
    (h, w) = image.shape[:2]
    w -= int(w % config.SCALE)
    h -= int(h % config.SCALE)
    image = image[0 : h, 0 : w]

    # to generate training images, we first need to downscale the image by
    # scaler factor and then upscale it back too the original size
    ...
    for y in range(0, h - config.INPUT_DIM + 1, config.STRIDE):
        for x in range(0, w - config.INPUT_DIM + 1, config.STRIDE):
            # crop output the 'INPUT_DIM x INPUT_DIM' ROI from our scaled image
            # this ROI will serve as the input to network
            crop = scaled[y : y + config.INPUT_DIM, x : x + config.INPUT_DIM]

            # crop out the 'LABEL_SIZE x LABEL_SIZE' ROI from our original image
            # this ROI will be the target output from network
            target = image[
